...
let rec transformToString =
    ...
    | Transform.Translate(x, y) -> sprintf "translate(%s, %s)" (lengthToString x) (lengthToString y)
    | Transform.Rotate deg -> sprintf "rotate(%ddeg)" deg
    | Transform.Multiple transforms -> transforms |> List.map transformToString |> String.concat " "
...
let rec transform3DToString =
    ...
    | Transform3D.Translate3d(x, y, z) ->
        sprintf "translate3d(%s, %s, %s)" (lengthToString x) (lengthToString y) (lengthToString z)
...
let rec shadowToString =
    ...
    | Shadow.Offset(x, y, color) -> sprintf "%s %s %s" (lengthToString x) (lengthToString y) color
